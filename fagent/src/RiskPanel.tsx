...
import { usePortfolio, fmtTL, fmtPct } from './store';
          ...
          {hasCoverage && level && (
            <>
              ...
              <p className="hint" style={{ marginBottom: 12, color: report.coveragePct < 60 ? 'var(--amber, #fbbf24)' : undefined }}>
                📊 Bu rakamlar portföyünün <strong style={{ color: 'var(--text)' }}>%{report.coveragePct.toFixed(0)}</strong>'ini kapsıyor
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14, marginBottom: 14 }}>
                <div>
                  ...
                  <div className="mono" style={{ fontSize: 17, fontWeight: 700, color: LEVEL_TEXT[level].color }}>
                    %{report.portfolioVolPct.toFixed(1)}
                  </div>
                ...
                <div>
                  ...
                  <div className="mono" style={{ fontSize: 17, fontWeight: 700, color: 'var(--red)' }}>
                    −%{report.portfolioDrawdownPct.toFixed(1)}
                  </div>
                ...
                {report.sharpe !== undefined && (
                  <div>
                    ...
                    <div className="mono" style={{ fontSize: 17, fontWeight: 700 }}>{report.sharpe.toFixed(2)}</div>
              ...
              {report.decomposition && report.decomposition.weightedAvgVolPct - report.portfolioVolPct > 0.1 && (
                <p className="hint" style={{ marginBottom: 12 }}>
                  ...
                  %{report.decomposition.weightedAvgVolPct.toFixed(1)} olurdu (hepsi birlikte hareket etseydi); gerçekte portföy volatiliten
                  %{report.portfolioVolPct.toFixed(1)}. Aradaki{' '}
                  <strong style={{ color: 'var(--accent)' }}>{(report.decomposition.weightedAvgVolPct - report.portfolioVolPct).toFixed(1)} puan</strong>,
              ...
              <div style={{ paddingTop: 12, borderTop: '1px solid var(--line)' }}>
                ...
                {report.assets.map(a => (
                  <div key={a.name} className="list-row">
                    <span>{a.name} <span className="badge">%{(a.weight * 100).toFixed(0)}</span></span>
                    <span className="mono sub">
                      vol %{a.volPct.toFixed(1)} · düşüş −%{a.drawdownPct.toFixed(1)}
                    </span>
                  </div>
                ))}
              </div>
